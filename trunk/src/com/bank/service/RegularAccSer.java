...
public class RegularAccSer extends AAccountSer implements IIndividualAccSer {
	...
	public OptRecord checkbalance(Long idNumber, Long accountId, String password) {
		...
		if(!regacc.getIsActive() || custloginfo == null || customer == null ||
		...
		record.setOptdesc(new String("The balance of acc: ")
			+regacc.getId()+" is "+regacc.getBalance().toString());
		record.setInAmt(0.0);
	...
	public OptRecord deposit(Long accountId, String password, double amount) {
		...
		if(!regacc.getIsActive() || custloginfo == null || 
				!(custloginfo.getPassword().equals(password))){
			record.setOptdesc(new String("Acc & pwd are not matched."));
			optrecordDao.save(record);
			return record;
		...
		record.setOptdesc(new String("Deposit amount ")+
				amount+", and total is: "+regacc.getBalance().toString());
	...
	public OptRecord destory(Long idNumber, Long accountId, String password) {
		...
		regaccDao.attachDirty(regacc);
		record.setOptdesc(new String("Acc: ")+regacc.getId()+
	...
	public OptRecord transfer(Long accountId, Long idNumber, String password,
		...
		if(regacc == null || iregacc == null) {
			record.setOptdesc(new String("Acc: ")+accountId+
					" and " + inAccountId + " do not exist");
			optrecordDao.save(record);
			return record;
		}
		...
		if(!regacc.getIsActive() || custloginfo == null || customer == null ||
			...
			!iregacc.getIsActive() || icustloginfo == null || icustomer == null ||
				!(icustomer.getName().equals(inCustomerName)) )	{
			record.setOptdesc(new String("Acc, id number & pwd are not matched."));
			optrecordDao.save(record);
			return record;
		}
		...
		if(remain < 0.0){
			record.setOptdesc(new String("Balance is not enough.")+
					regacc.getBalance()+" - "+String.valueOf(amount));
			optrecordDao.save(record);
			return record;
		}
		...
		record.setOptdesc(new String("transfer amount from id-")+regacc.getId().toString() 
				+ " to "+ iregacc.getId().toString() + " " + String.valueOf(amount));
	...
	public OptRecord withdraw(Long accountId, String password, double amount) {
		...
		record.setOptdesc(new String("Draw balance: ")+
				String.valueOf(amount)+" and remain: "+regacc.getBalance());
